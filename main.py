...
async def send_log(action_type, moderator, target_user, old_rank, new_rank):
    try:
        ...
        embed.add_field(name="Target User", value=str(target_user), inline=True)
        embed.add_field(name="Old Rank", value=str(old_rank), inline=False)
        embed.add_field(name="New Rank", value=str(new_rank), inline=False)
...
async def promote_command(interaction: discord.Interaction, username: str):
    ...
    if promote_user(user_id, next_role):
...
async def demote_command(interaction: discord.Interaction, username: str):
    ...
    if promote_user(user_id, prev_role):
...
async def setrank_command(interaction: discord.Interaction, username: str, rank: app_commands.Choice[str]):
    ...
    current_role, _ = get_user_current_role(user_id)
    if promote_user(user_id, rank.value):
        ...
        await interaction.followup.send(embed=discord.Embed(description="❌ Failed to set rank.", color=discord.Color.red()))
